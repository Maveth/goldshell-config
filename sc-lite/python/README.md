...
| **`smooth`** | Continuous ramp (weighted history). **Fiddle** `min_temp/max_temp/min_fan/max_fan`. **Not fully tested yet** (experimental). | Keep **ON** — `tempcontrol=false` does **not** stop the stock 85°C walk (verified 2026-09-22); use abort + `restore_auto_on_exit` |
```powershell
...
- Keep `tempcontrol` on (default) — OFF does **not** disable the fan walk-down
- Abort **90°C** by default
- Don’t leave `tempcontrol` off tests unattended (overheat-path role still unclear)
