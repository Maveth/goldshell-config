...
SC Lite tools live in [`../sc-lite/`](../sc-lite/). Use **[`python/`](python/)** in *this* directory for HS Box — same HTTP/JWT flow, different powerplan string grammar.

## Status

| Item | State |
|------|--------|
| Fan field adjust via `PUT /mcb/setting` | **Confirmed** (live HS Box — **Hersh-23**) |
| Login / snapshot / fan-kick helpers | **Confirmed** — see [`python/`](python/) |
| Powerplan string dialect (float V / optional PV) | **Confirmed** — e.g. `750 MHz 0.41 V 50 RPM 50 RPM` |
| Clock / voltage changes | **Not recommended** — leave MHz/V alone unless you accept ASIC risk |
| Soft-restart / put-fail recovery | Same idea as SC Lite (`GET /mcb/restart`); not yet wired into this tree’s temp manager copy |

## Credit

- **Hersh-23** — verified live HS Box: JWT login, snapshot, fan-only kick; authored [`python/`](python/) as a sibling of `sc-lite/python` with HS Box `parse_plan` / `build_plan` (decimal V, optional PV). Upstream fork: https://github.com/Hersh-23/goldshell-config
- **BitcoinMechanic** — original SC Lite reclock / control-plane discovery
- **MaVeTh** — SC Lite fan/temp docs and Python tooling that Hersh adapted
